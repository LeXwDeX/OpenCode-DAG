...
#### WP-D2 — worker_type="dag" 分发 ✅ 已完成

`worker_type="dag"` 成为保留字（运行时校验，不改 TS 类型声明 — 地基 `types.ts:193` 是 `worker_type: string` 非 union；INFO-1 处理）。`validateWorkerTypes` 在 `core-start.ts:66-89` 校验循环内对 "dag" 显式 skip + 三重校验（subDagConfig 缺失拒绝 + subDagConfig 非法拒绝 + agent registry 含 "dag" key 冲突拒绝）。`spawnReadyNode` 内 L530-629 插入 dispatch 段（agent resolve L633+ 之前短路；archgate 约束 2 honored）：提取 `worker_config.subDagConfig` → 校验合法性（null/非 object/非 array nodes 全部 fail 节点）→ 计算 `childDepth = parentDepth + 1`（parentDepth 从 `parentWf.metadata.depth` 读取）→ 创建 child session（INFO-5：chatSessionId = `subChildSession.id`，与 agent-type 节点同模式）→ 节点状态 running → 调用 `bootstrapWorkflowFromConfig({..., parentWorkflowId, parentNodeId, depth: childDepth})` → 失败时 fail 节点 + createViolation(type: "subdag_depth_exceeded", details: {depth, max: MAX_SUB_DAG_DEPTH})。**递归深度 ≤3 schema 强制**：depth 校验在 `bootstrapWorkflowFromConfig` Step 0（单一来源）+ spawnReadyNode dispatch fail 节点双重守卫；depth DB 持久化路径：core-start.ts Step 3 createWorkflow 调用 `metadata: depth !== undefined ? { depth } : undefined`；spawnReadyNode 读取 parentWf.metadata.depth + 1 传递给 core 函数。`bootstrapWorkflowFromConfig` 签名扩展 parentWorkflowId? / parentNodeId? / depth? 三个可选参数（additive，dagworker.ts 现有工具路径不传则默认 undefined / depth=0，向后兼容）。四铁律逐图守恒（子工作流经 `createWorkflow → validateWorkflowConfigLimits` 独立校验节点 ≤20 / 并发 ≤10，天然守恒）。"dag" 节点 spawn 后保持 running 状态等待 WP-D3 事件桥（不越界 WP-D3）。新增 `SubDagWorkerConfig` interface（types.ts）+ `DAG_VIOLATION_TYPES` 扩展 "subdag_depth_exceeded" + `MAX_SUB_DAG_DEPTH = 3` const（limits.ts）+ i18n.ts 双语 label。review P0 修复：第一次实现因漏持久化 `metadata.depth` 导致 depth cap 运行时完全失效（spawnReadyNode 读取 parentDepth 始终 0，子 DAG 可无限嵌套）；修复在 core-start.ts Step 3 createWorkflow 加 `metadata: { depth }` + scenario-27a 添加 Test E 端到端验证（bootstrapWorkflowFromConfig({depth:3}) → DB 读 workflow.metadata.depth === 3）。8 unit tests（subdag-dispatch.test.ts：dag skip / reserved-word conflict / missing subDagConfig / invalid subDagConfig / agent accept / unregistered agent reject / depth 4 reject / depth 3 pass）+ 5 DB 集成测试（scenario-27a：workflow DB 行 + 21 nodes 拒绝 + depth 4 拒绝 + chat_session_id 持久化 + metadata.depth 端到端）。第一次实现回归 bug 教训：implement 误删 338 行 workflow-engine.ts 内容（包括 setWorkflowConcurrency export + 所有 spawnReadyNode/handleNodeCompletion/handleNodeFailure/getWorkflowStatus/scheduleReadyNodes/createWorkflowExecutor 闭包函数），按"回滚纪律"完全回滚到 WP-D1 commit 45c177350 + 删除损坏测试文件，第二次实现严格"仅 addition +103/-0 不删除任何现有行"纪律成功。archgate 7 项约束全 honored：四铁律逐图守恒 / dispatch 在 agent resolve 之前 / validateWorkerTypes "dag" skip / 递归深度 ≤3 schema 强制 + metadata.depth DB 持久化 / 签名扩展 / 不越界 WP-D3 / "dag" 保留字冲突检测。INFO 1-5 全处理。验收：subdag-dispatch 8/8 + scenario-27a 5/5 + dagworker 3/3 + core-start 4/4 + DAG session 386/386 + DAG core 53/53 + typecheck 0 errors。review INFO 4 项 P2 级（depth !== undefined ? { depth } : undefined 的 undefined 分支 dead code 因为 depth = 0 默认 / parentWf.chat_session_id as SessionID 强转 branded type / AbortController 独立不级联 WP-D3 处理 / Result.getFailure Option<unknown> 处理建议统一为 Effect.match）不阻塞。
