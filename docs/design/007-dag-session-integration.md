...
4. **⭐ 用户确认的三层架构**：
   ...
   - **执行层 Hook 驱动**：`/dagworker` slashcommand 直接走 DAG 新执行线（非 LLM 调用）
...
| 注入时机 | 注入内容 | 触发条件 |
|---------|---------|---------|
| **DAG 启动** | workflow_id、节点数量、配置参数 | `/dagworker create` 执行后 |
...
| **触发原因** | `/dagworker` 的参数 | 如 `/dagworker create refactor-module` |
...
export interface DAGContext {
  ...
  trigger_reason: string               // /dagworker 的参数
...
│   ├── tools/
│   │   └── dag-worker-command.ts      # /dagworker slashcommand handler（§3 偏离1）
...
│       ├── dag-worker-command.test.ts # 偏离1: /dagworker 触发器测试
