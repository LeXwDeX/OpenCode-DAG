...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
      ...
      for (const item of yield* registry.tools({
        ...
        const schema = ProviderTransform.schema(input.model, EffectZod.toJsonSchema(item.parameters) as JSONSchema7)
        tools[item.id] = tool({
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const tools = yield* SessionTools.resolve({
              ...
              Effect.provideService(SettingsHook.Service, settingsHook), // [FORK:tool-hooks]
            )
            ...
            const [skills, env, hooks, dag, instructions, modelMsgs] = yield* Effect.all([
              sys.skills(agent),
            ...
            const system = [...env, ...hooks, ...dag, ...instructions, ...(skills ? [skills] : [])]
