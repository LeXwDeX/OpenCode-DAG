              ...
              return run.promise(
                Effect.gen(function* () {
                  ...
                  const output = {
                    ...result,
                    ...(hookContexts.length && {
                      output: [result.output, ...hookContexts.map((c) => `<hook_additional_context>\n${c}\n</hook_additional_context>`)].join("\n\n"),
                    }),
          ...
          item.execute = (args, opts) =>
            run.promise(
              Effect.gen(function* () {
                ...
                // ── Settings hooks: PreToolUse (MCP tools) ──
                ...
                if (preHook.blocked) {
                  return {
                    ...
                    output: `Hook blocked: ${preHook.blocked.reason}`,
                    attachments: [],
                    content: [{ type: "text" as const, text: `Hook blocked: ${preHook.blocked.reason}` }],
                  }
                }
                ...
                // ── Settings hooks: PostToolUse (MCP tools) ──
                ...
                // Append hook additionalContext to text parts
                const hookContexts = [...preHook.additionalContexts, ...postHook.additionalContexts]
                if (hookContexts.length) {
                  textParts.push(...hookContexts.map((c) => `<hook_additional_context>\n${c}\n</hook_additional_context>`))
                }
