...
describe("WP4 i18n — t() dict lookup", () => {
  const cases: Record<I18nKey, [string, string]> = {
    ...
    dlg_replan_title: ["Replan workflow", "重新规划工作流"],
    dlg_replan_ph: ["1-10", "1-10"],
    replan_concurrency: ["Change Concurrency", "修改并发数"],
    replan_add_node: ["Add Node", "添加节点"],
    replan_remove_node: ["Remove Node", "移除节点"],
    replan_update_node: ["Update Node Config", "更新节点配置"],
    dlg_concurrency_title: ["Set Max Concurrency (1-10)", "设置最大并发数 (1-10)"],
    dlg_concurrency_ph: ["Enter number 1-10", "输入 1-10 的数字"],
    dlg_add_node_title: ["Add Node (JSON config)", "添加节点（JSON 配置）"],
    dlg_add_node_ph: ['{"id":"new","dependencies":["A"],"worker_type":"general","worker_config":{"prompt":"task"}}', '{"id":"new","dependencies":["A"],"worker_type":"general","worker_config":{"prompt":"task"}}'],
    dlg_remove_node_title: ["Select Node to Remove", "选择要移除的节点"],
    dlg_update_node_title: ["Select Node to Update", "选择要更新的节点"],
    dlg_update_node_prompt: ["Update Node Config (JSON patch)", "更新节点配置（JSON 补丁）"],
    dlg_update_node_ph: ['{"worker_config":{"prompt":"new task"}}', '{"worker_config":{"prompt":"new task"}}'],
    dlg_confirm_remove_title: ["Confirm Removal", "确认移除"],
    dlg_confirm_remove_msg: ["Remove node '{node}'? Running nodes will be cascaded.", "移除节点 '{node}'？运行中节点将被级联影响。"],
    toast_node_added: ["Node added", "节点已添加"],
    toast_node_removed: ["Node removed", "节点已移除"],
    toast_node_updated: ["Node updated", "节点已更新"],
    toast_invalid_json: ["Invalid JSON format", "JSON 格式无效"],
    toast_missing_node_id: ["Node config requires 'id' field", "节点配置需要 'id' 字段"],
    toast_no_nodes: ["No nodes available", "没有可用节点"],
    toast_replan_error: ["Replan failed", "重规划失败"],
