  ...
  Effect.gen(function* () {
        ...
        yield* bus.subscribe(SessionStatus.Event.Status).pipe(
          ...
          Stream.runForEach((evt) =>
            Effect.gen(function* () {
              ...
              const st = evt.properties.status
              if (st.type !== "idle") return
              const cause = st.cause
              if (cause === "abort") {
                yield* goal.pause(sid, "当前轮被中断").pipe(Effect.ignore)
                return
              }
              if (cause === "error") return
              // cause === "complete" or undefined (legacy compat)
    ...
    const afterIdle = Effect.fn("GoalLoop.afterIdle")(function* (sessionID: SessionID) {
      ...
      if (currentStatus.type !== "idle") {
        Log.Default.info("GoalLoop: session no longer idle, skipping continuation", { sessionID })
        return
      }

      if (shouldPreempt(msgs)) {
        Log.Default.info("GoalLoop: user message detected after last assistant, pausing goal", { sessionID })
        yield* goal.pause(sessionID, "当前轮被中断").pipe(Effect.ignore)
        return
      }
