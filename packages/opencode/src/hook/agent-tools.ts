...
const log = Log.create({ service: "hook.agent-tools" })
    ...
    execute: async (args: any) => {
      try {
        ...
        const walk = async (dir: string, rel: string): Promise<void> => {
          const entries = await Effect.runPromise(fs.readDirectoryEntries(dir) as Effect.Effect<AppFileSystem.DirEntry[], unknown>)
