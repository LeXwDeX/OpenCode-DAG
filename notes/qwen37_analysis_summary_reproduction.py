#!/usr/bin/env python3
"""
Qwen3.7-Max <analysis>/<summary> 标签生成问题复现脚本

基于实际会话记录 ses_1a346511dffeMuGlOGc3t4LOC8 复现问题。
使用阿里云百炼直接 API，保持当时的参数（variant=high）。
"""
...
# 日志文件
LOG_FILE = f"qwen37_analysis_summary_reproduction_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def get_system_prompt_with_dcp_context():
    """获取包含 DCP 上下文的系统提示词"""
    # 模拟当时的系统提示词，包含 DCP 相关内容
...
def simulate_conversation_with_analysis_summary():
    """模拟对话，尝试触发 <analysis>/<summary> 标签生成"""
    
    system_prompt = get_system_prompt_with_dcp_context()
    ...
    log("=" * 80)
    log(f"开始 <analysis>/<summary> 标签复现测试")
    log(f"模型: {MODEL}")
    log(f"API: {BASE_URL}")
...
def main():
    """主函数"""
    print("Qwen3.7-Max <analysis>/<summary> 标签生成问题复现脚本")
    print("=" * 80)
    print("此脚本将执行 30 轮多轮对话，使用 variant=high (thinking depth)")
    print("尝试触发 <analysis>/<summary> 标签生成问题")
    print("=" * 80)
    
    anomaly, total, tags = simulate_conversation_with_analysis_summary()
