...
function build(key: string, remote: Item, url: string, prev?: Model): Model {
  ...
  // TODO(refactor-trigger): 此处是 Anthropic 路由判别的 success path（最权威，基于
  // 上游告知的 supported_endpoints）。同结构 fallback 启发式判别在 copilot.ts:fix()
  // 与 github-proxy/proxy.ts:fix() 各 1 处，共 3 处。再增第 4 处或 fallback 正则失守时，
  // 抽 plugin/github-copilot/routing.ts 统一收敛（success path 仍优先于启发式）。
