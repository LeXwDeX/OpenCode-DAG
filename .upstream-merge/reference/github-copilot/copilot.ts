...
function fix(model: Model, url: string, isEnterprise = false): Model {
  // Even when /copilot/models lookup fails and we fall back here, Claude must keep
  // routing through the Anthropic messages API; otherwise Copilot's OpenAI-style
  // endpoint rejects it with model_not_supported (and downstream we hit the
  // thinking-signature schema issue). See
  // .memory/errors/github-proxy-thinking-signature-schema-fallback-20260421.md
  // TODO(refactor-trigger): 此处 Anthropic 路由判别在本仓 fork 已第 3 次出现
  // (此文件 / models.ts:59,66-67 / github-proxy/proxy.ts:16-30)。下次再新增第 4 处，
  // 或上游推出无 claude/opus/sonnet/haiku 字样的新 Anthropic 模型导致此正则失守时，
  // 应抽取 plugin/github-copilot/routing.ts 提供 isAnthropicMessagesModel +
  // applyAnthropicRouting，3 处统一收敛。背景见
  // .memory/patterns/opencode-plugin-fallback-signature-preservation-20260421.md
  //
  // Enterprise host (copilot-api.<domain>) 上 messages API 是否可用、路径前缀
  // 是否相同都未知；fallback 时不应推测，统一只替换 host 走 OpenAI-compatible
  // 兼容端点（@ai-sdk/github-copilot），不追加 /v1 也不切换到 anthropic 路由。
  const isClaude = /claude|opus|sonnet|haiku/i.test(model.api.id)
  const useAnthropicRoute = isClaude && !isEnterprise
  return {
    ...model,
    api: {
      ...model.api,
      url: useAnthropicRoute ? `${url}/v1` : url,
      npm: useAnthropicRoute ? "@ai-sdk/anthropic" : "@ai-sdk/github-copilot",
    },
  }
}
        ...
        return CopilotModels.get(
          ...
          return Object.fromEntries(
            Object.entries(provider.models).map(([id, model]) => [
              id,
              fix(model, base(auth.enterpriseUrl), !!auth.enterpriseUrl),
            ]),
          )
        })
      },
