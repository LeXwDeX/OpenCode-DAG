...
- **🪝 Hooks API のスーパーセット**：Claude Code の 22 種類のトリガーイベント × 5 種類の実行タイプを基盤に、このフォークは **Claude Code Hooks プロトコルと完全互換**であり、さらに DAG ワークフローのイベントバス連携（`workflow.*` / `node.*` イベント）、TUI サブスクリプション、HTTP API 転送を追加しています。完全な仕様は [`hooks-reference.md`](./packages/opencode/src/session/prompt/hooks-reference.md) を参照してください
- **🎯 Goal 指示システム**：`todowrite` ツールと構造化された目標追跡により、長時間の多段階 agent セッションでも作業キューを保持し、コンテキストウィンドウの変化によるタスク状態の喪失を防ぎます
- **🪝 TODO PreHook**：`PreToolUse` hook 経由で TODO リストをコンテキストへ注入できます。hooks 駆動の goal reentry により、agent は常に現在の進捗を確認できます
- **🛡️ Sandbox Coding Workspace**：各 sandbox は独立した一時ディレクトリ、LSP 診断、多言語 toolchain（Python/Node/TS/Go/Rust/C/C++）を備えます。agent は隔離環境でコードを試行・コンパイル・デバッグし、検証後に edit/write でプロジェクトファイルへ反映できます

### 公式 opencode との違いは？

- **🪝 Hooks API のスーパーセット + Goal 指示 + TODO PreHook + Sandbox Workspace**：上流の Hooks 機能をすべて維持しつつ、DAG イベント連携、構造化タスク追跡、hooks 駆動の goal reentry、多言語の隔離 coding sandbox を追加しています
- **🧩 DAG WorkFlow モード（WIP・約 90% 完了）**：自社開発の [Harness-DAG-Workflow](./docs/harness-dag.md) エンジンにより、LLM agent が 1 つのセッション内で複数ノードの並列タスクをオーケストレーションできます。主要機能（スケジューリング / ライフサイクル / pause-resume-cancel-replan-step / sub-DAG / 条件分岐 / data flow / crash recovery / probes）は実装済みで、TUI パネルも接続済み、残りは仕上げ段階です
- **🔧 中国語互換性修正**：上流由来の CJK tokenization、全角句読点、中国語パス、IME edge cases を継続的に DEBUG しています
